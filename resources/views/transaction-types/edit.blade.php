<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl text-surface-800 leading-tight">Edit: {{ $transactionType->name }}</h2>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('transaction-types.update', $transactionType) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="description" name="description" rows="2"
                            class="mt-1 block w-full border-surface-300 rounded-md shadow-sm focus:ring-brand-500 focus:border-brand-500 text-sm">{{ old('description', $transactionType->description) }}</textarea>
