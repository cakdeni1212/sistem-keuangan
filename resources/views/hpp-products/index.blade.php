<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('hpp-products.import') }}"
               class="btn-secondary">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                Import Excel
            </a>
            <a href="{{ route('hpp-products.create') }}"
                class="btn-primary">
    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="stat-card text-center">
                ...
                <p class="page-title">{{ $products->count() }}</p>
            </div>
            <div class="stat-card text-center">
                ...
                <p class="text-xl font-bold text-emerald-600">{{ $products->where('is_active', true)->count() }}</p>
            </div>
            <div class="stat-card text-center">
                ...
                <p class="text-xl font-bold {{ $avgMargin >= 100 ? 'text-emerald-600' : ($avgMargin >= 50 ? 'text-amber-600' : 'text-red-600') }}">
                ...
                <table class="table-wrap">
                    ...
                    <tbody class="divide-y divide-surface-100">
                        ...
                        <tr class="hover:bg-surface-50 transition-colors">
                            <td class="table-td text-surface-300 text-xs">{{ $i + 1 }}</td>
                            <td class="table-td font-semibold text-surface-900">
                                {{ $p->name }}
                                @if($p->notes)
                                <p class="text-xs text-surface-400 font-normal mt-0.5">{{ Str::limit($p->notes, 50) }}</p>
                                @endif
                            </td>
                            <td class="table-td text-surface-400 text-xs font-mono">{{ $p->sku ?? '—' }}</td>
                            <td class="table-td">{{ $p->category ?? '—' }}</td>
                            <td class="table-td text-sm">{{ $p->satuan ?? '—' }}</td>
                            <td class="table-td text-right text-surface-600">Rp {{ number_format($p->bahan_baku, 0, ',', '.') }}</td>
                            <td class="table-td text-right font-semibold text-surface-900">Rp {{ number_format($p->hpp_total, 0, ',', '.') }}</td>
                            <td class="table-td text-right font-semibold text-brand-700">Rp {{ number_format($p->harga_jual, 0, ',', '.') }}</td>
                            <td class="table-td text-right">
                                <span class="{{ $marginBadge }}">{{ number_format($marginPct, 1) }}%</span>
                                <p class="text-[10px] text-surface-400 mt-1 text-right">Rp {{ number_format($p->margin_amount, 0, ',', '.') }}</p>
                            </td>
                            ...
                            <td class="table-td text-center">
                                <div class="flex items-center justify-center gap-1">
                                    @can('edit hpp')
                                    <a href="{{ route('hpp-products.edit', $p) }}"
                                    ...
                                    @endcan
                                    @can('delete hpp')
                                    <form action="{{ route('hpp-products.destroy', $p) }}" method="POST"
                        ...
                        <tr>
                            <td colspan="11" class="px-6 py-12 text-center text-sm text-surface-400">
                                Belum ada data produk. <a href="{{ route('hpp-products.create') }}" class="text-brand-600 font-semibold hover:underline">Tambah sekarang</a>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
