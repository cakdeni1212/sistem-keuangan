<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="page-title">Tambah Cashbon</h2>
        </div>
    </x-slot>
    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8">
        <div class="max-w-2xl mx-auto">
            ...
            <form action="{{ route('cashbons.store') }}" method="POST" class="space-y-5"
                  enctype="multipart/form-data"
                  x-data="{ debtorType: '{{ old('debtor_type', 'lainnya') }}' }">
                @csrf
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="debtor_name" id="debtor_name"
                                list="debtor-name-list"
                                value="{{ old('debtor_name') }}" required
                                autocomplete="off"
                                placeholder="Ketik atau pilih nama peminjam"
                        ...
                        <div>
                            ...
                            <select name="debtor_type" x-model="debtorType" required
                                class="w-full border border-surface-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
                                <option value="karyawan" {{ old('debtor_type') === 'karyawan' ? 'selected' : '' }}>Karyawan</option>
                                <option value="pelanggan" {{ old('debtor_type') === 'pelanggan' ? 'selected' : '' }}>Pelanggan</option>
                                <option value="supplier" {{ old('debtor_type') === 'supplier' ? 'selected' : '' }}>Supplier</option>
                                <option value="lainnya" {{ old('debtor_type', 'lainnya') === 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                            </select>
                        </div>
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="amount" value="{{ old('amount') }}" required min="0" step="any"
                                    placeholder="0"
                ...
                <div class="bg-brand-50 border border-brand-100 rounded-xl p-4">
                    <p class="text-xs text-brand-600">ℹ️ Saat cashbon disimpan, otomatis dibuat transaksi pengeluaran <strong>"Cashbon Keluar"</strong> dengan status Disetujui.</p>
                </div>
                ...
                <div class="stat-card space-y-3" x-data="receiptUploader()">
                    ...
                    <template x-if="preview">
                        <div class="relative">
                            ...
                            <p class="text-xs text-surface-400 mt-1">Klik &#215; untuk menghapus pilihan.</p>
                        </div>
                    </template>
                ...
                <div class="flex gap-3">
                    <button type="submit" class="px-6 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700 font-medium text-sm transition">Simpan Cashbon</button>
