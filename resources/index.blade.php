<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-gray-800">👥 Karyawan</h2>
                <p class="text-xs text-gray-500 mt-0.5">Data dan rekap gaji seluruh karyawan</p>
            </div>
            @can('create employee')
            <a href="{{ route('employees.create') }}" class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition">
                + Tambah Karyawan
            </a>
            @endcan
        </div>
    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8 space-y-5">
        ...
        <div class="bg-white rounded-xl border shadow-sm px-5 py-4">
            <form method="GET" action="{{ route('employees.index') }}" id="top-filter-form"
                ...
                <span class="text-sm font-semibold text-gray-600 mr-1">🗓 Filter Periode:</span>
                ...
                <a href="{{ route('employees.index', ['gaji_month' => $thisM, 'gaji_year' => $thisY, 'rfm' => $rfm, 'rfy' => $rfy, 'rtm' => $rtm, 'rty' => $rty]) }}"
                   class="px-3 py-1 text-xs rounded-full border transition {{ ($filterMonth == $thisM && $filterYear == $thisY) ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-300 hover:border-indigo-400' }}">
                    Bulan Ini
                </a>
                <a href="{{ route('employees.index', ['gaji_month' => $prevM, 'gaji_year' => $prevY, 'rfm' => $rfm, 'rfy' => $rfy, 'rtm' => $rtm, 'rty' => $rty]) }}"
                   class="px-3 py-1 text-xs rounded-full border transition {{ ($filterMonth == $prevM && $filterYear == $prevY) ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-300 hover:border-indigo-400' }}">
                    Bulan Lalu
                </a>
                @foreach($periods->take(4) as $p)
                    ...
                    <a href="{{ route('employees.index', ['gaji_month' => $p->period_month, 'gaji_year' => $p->period_year, 'rfm' => $rfm, 'rfy' => $rfy, 'rtm' => $rtm, 'rty' => $rty]) }}"
                       class="px-3 py-1 text-xs rounded-full border transition hidden sm:inline-block {{ $isActive ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-300 hover:border-indigo-400' }}">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            <div class="bg-white rounded-xl border shadow-sm p-4 flex items-center gap-3 col-span-1">
                <div class="w-9 h-9 bg-indigo-100 rounded-lg flex items-center justify-center text-lg flex-shrink-0">👥</div>
                <div>
                    <p class="text-xs text-gray-500">Total</p>
                    <p class="text-xl font-extrabold text-gray-800">{{ $employees->count() }}</p>
                </div>
            </div>
            <div class="bg-white rounded-xl border shadow-sm p-4 flex items-center gap-3 col-span-1">
            ...
            <div class="bg-white rounded-xl border shadow-sm p-4 flex items-center gap-3 col-span-2 sm:col-span-1">
                <div class="w-9 h-9 bg-purple-100 rounded-lg flex items-center justify-center text-lg flex-shrink-0">💸</div>
                <div class="min-w-0">
                    <p class="text-xs text-gray-500 truncate">Total Gaji <span class="font-medium text-purple-600">{{ $gajiPeriodLabel }}</span></p>
            ...
            <div class="bg-white rounded-xl border shadow-sm p-4 flex items-center gap-3 col-span-1">
                ...
                <div class="min-w-0">
                    <p class="text-xs text-gray-500">Dibayar <span class="text-emerald-600 font-medium">{{ $gajiPeriodLabel }}</span></p>
                    <p class="text-base font-extrabold text-emerald-700">Rp {{ number_format($totalGajiDibayar, 0, ',', '.') }}</p>
                    <p class="text-xs text-gray-400">{{ $countDibayar }} org</p>
                </div>
            </div>
            <div class="bg-white rounded-xl border shadow-sm p-4 flex items-center gap-3 col-span-1">
                <div class="w-9 h-9 bg-amber-100 rounded-lg flex items-center justify-center text-lg flex-shrink-0">⏳</div>
                <div class="min-w-0">
                    <p class="text-xs text-gray-500">Belum Bayar <span class="text-amber-600 font-medium">{{ $gajiPeriodLabel }}</span></p>
                    <p class="text-base font-extrabold text-amber-700">Rp {{ number_format($totalGajiBelumBayar, 0, ',', '.') }}</p>
                    <p class="text-xs text-gray-400">{{ $countBelumBayar }} org</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 rounded-xl p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="text-center">
                    ...
                    <p class="text-2xl font-extrabold text-gray-800">{{ $pctDibayar }}%</p>
                    <p class="text-xs text-gray-500 mt-1">Rasio Pembayaran</p>
                    <div class="mt-2 w-full max-w-xs mx-auto bg-gray-200 rounded-full h-2">
        ...
        <div class="bg-white rounded-xl border shadow-sm overflow-hidden">
            <div class="px-5 py-3.5 border-b flex items-center justify-between">
                <h3 class="text-sm font-bold text-gray-700">Daftar Karyawan</h3>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-semibold text-gray-600 hidden md:table-cell">No HP</th>
                            <th class="px-4 py-3 text-right font-semibold text-gray-600 hidden md:table-cell">Gaji Pokok</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-4 py-3">
                                <a href="{{ route('employees.show', $emp) }}" class="font-medium text-indigo-700 hover:underline">{{ $emp->name }}</a>
                                @if($emp->department)
                                <span class="block text-xs text-gray-400">{{ $emp->department }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-gray-700">{{ $emp->position }}</td>
                            <td class="px-4 py-3 text-gray-500 hidden md:table-cell">{{ $emp->phone ?? '—' }}</td>
                            <td class="px-4 py-3 text-right text-gray-700 font-medium hidden md:table-cell">
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('employees.show', $emp) }}" class="text-xs text-indigo-600 hover:underline">Detail</a>
                                    @can('edit employee')
        ...
        <div class="bg-white rounded-xl border shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h3 class="text-sm font-bold text-gray-700">📊 Rekap Kumulatif Gaji</h3>
                    <p class="text-xs text-gray-400 mt-0.5">Total gaji termasuk bulan-bulan sebelumnya</p>
                </div>
                <form method="GET" action="{{ route('employees.index') }}" class="flex flex-wrap items-center gap-2" id="range-form">
                    ...
                    <select name="rfm" onchange="document.getElementById('range-form').submit()"
                            class="border-gray-300 rounded-lg text-sm py-1.5">
                        @foreach($months as $n => $name)
                    ...
                    <span class="text-xs text-gray-500 font-medium">Sampai</span>
            ...
            <div class="px-5 py-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="bg-gray-50 rounded-xl p-4 text-center border">
                    ...
                    <p class="text-xl font-extrabold text-gray-800">Rp {{ number_format($rangeTotalAll, 0, ',', '.') }}</p>
                    @if($rangeChartLabels)
                    <p class="text-xs text-gray-400 mt-1">{{ count($rangeChartLabels) }} periode</p>
                    @endif
                </div>
            ...
            @else
            <div class="px-5 pb-6 text-center text-sm text-gray-400">Tidak ada data gaji pada rentang ini.</div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl border shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h3 class="text-sm font-bold text-gray-700">💸 Rekap Gaji — {{ $gajiPeriodLabel }}</h3>
                    <p class="text-xs text-gray-400 mt-0.5">Detail gaji seluruh karyawan periode ini</p>
                </div>
                @if($salaryRecords->count() > 0)
                <a href="{{ route('laporan.export-gaji', ['month' => $filterMonth, 'year' => $filterYear]) }}"
                   class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white text-xs font-medium rounded-lg hover:bg-purple-700 transition">
            ...
            <div class="py-12 text-center text-gray-400">
                ...
                <p class="text-xs mt-1">
                    ...
                    <a href="{{ route('employees.index') }}" class="text-indigo-600 hover:underline">Lihat daftar karyawan</a>
                </p>
                @endcan
            </div>
            @else
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center font-semibold text-gray-600">Status</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-600 hidden md:table-cell">Tgl Bayar</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-600">Slip</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-4 py-3">
                                <a href="{{ route('employees.show', $sal->employee) }}" class="font-medium text-indigo-700 hover:underline">
                                    {{ $sal->employee->name }}
                                </a>
                                <span class="block text-xs text-gray-400">{{ $sal->employee->position ?? '' }}</span>
                            </td>
                            <td class="px-4 py-3 text-right text-gray-700">Rp {{ number_format($sal->base_salary, 0, ',', '.') }}</td>
                            ...
                            <td class="px-4 py-3 text-right font-bold text-gray-800">Rp {{ number_format($sal->total_salary, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-center">
                                @if($sal->paid_at)
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                <a href="{{ route('employee-salaries.slip', [$sal->employee, $sal]) }}"
                                   class="inline-flex items-center gap-1 px-2 py-1 text-xs text-indigo-700 bg-indigo-50 border border-indigo-200 rounded hover:bg-indigo-100 transition">
                                    🧾 Slip
                                </a>
                                @endcan
                            </td>
                    ...
                    <tfoot class="border-t-2 border-gray-200 bg-gray-50">
                        <tr>
                            <td class="px-4 py-3 font-semibold text-gray-600 text-sm">
