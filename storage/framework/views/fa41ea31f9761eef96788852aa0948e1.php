...
<?php $component->withAttributes([]); ?>
    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8 space-y-4">
        ...
        <div class="bg-white dark:bg-surface-800 rounded-2xl border border-surface-200 dark:border-surface-700 shadow-sm px-5 py-4">
            <form method="GET" action="<?php echo e(route('pengeluaran.index')); ?>" id="filter-form"
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <?php if($filterDate): ?>
                        <a href="<?php echo e(route('pengeluaran.index', ['month' => $month, 'year' => $year])); ?>"
                ...
                ?>
                <a href="<?php echo e(route('pengeluaran.index', ['month' => $nowM, 'year' => $nowY])); ?>"
                   class="px-3 py-1 text-xs rounded-full border transition <?php echo e(($month == $nowM && $year == $nowY && !$filterDate) ? 'bg-brand-600 text-white border-brand-600' : 'bg-white dark:bg-surface-700 text-surface-600 dark:text-surface-300 border-surface-300 dark:border-surface-600 hover:border-brand-400'); ?>">
                    Bulan Ini
                </a>
                <a href="<?php echo e(route('pengeluaran.index', ['month' => $prevM, 'year' => $prevY])); ?>"
                   class="px-3 py-1 text-xs rounded-full border transition <?php echo e(($month == $prevM && $year == $prevY && !$filterDate) ? 'bg-brand-600 text-white border-brand-600' : 'bg-white dark:bg-surface-700 text-surface-600 dark:text-surface-300 border-surface-300 dark:border-surface-600 hover:border-brand-400'); ?>">
                    Bulan Lalu
                </a>
                <a href="<?php echo e(route('pengeluaran.index', ['month' => $prev2M, 'year' => $prev2Y])); ?>"
                   class="px-3 py-1 text-xs rounded-full border transition hidden sm:inline-block <?php echo e(($month == $prev2M && $year == $prev2Y && !$filterDate) ? 'bg-brand-600 text-white border-brand-600' : 'bg-white dark:bg-surface-700 text-surface-600 dark:text-surface-300 border-surface-300 dark:border-surface-600 hover:border-brand-400'); ?>">
                    <?php echo e($months[$prev2M]); ?> <?php echo e($prev2Y); ?>

                </a>

                
                <?php if($totalAllCount > 0): ?>                <a href="<?php echo e(route('laporan.export-transaksi', ['month' => $month, 'year' => $year, 'category' => 'pengeluaran'])); ?>"
        ...
        <div class="bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 border border-red-100 dark:border-red-800 rounded-xl shadow-sm p-5">
            <p class="text-xs font-bold text-red-500 dark:text-red-400 uppercase tracking-widest mb-4">💸 Ringkasan Pengeluaran — <?php echo e($periodLabel); ?></p>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="bg-white/70 dark:bg-red-900/20 border border-red-100 dark:border-red-800 rounded-xl px-4 py-3 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-extrabold text-red-700 dark:text-red-300 leading-tight">Rp <?php echo e(number_format($totalPengeluaran, 0, ',', '.')); ?></p>
                        <?php if($pctChange !== null): ?>
                        <p class="text-xs font-semibold <?php echo e($selisih >= 0 ? 'text-red-500' : 'text-green-500'); ?>">
                            <?php echo e($selisih >= 0 ? '▲' : '▼'); ?> <?php echo e(number_format(abs($pctChange), 1)); ?>% vs bln lalu
                        </p>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="bg-white/70 dark:bg-yellow-900/20 border border-yellow-100 dark:border-yellow-800 rounded-xl px-4 py-3 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-extrabold text-yellow-600 dark:text-yellow-300 leading-tight">Rp <?php echo e(number_format($pendingTotal, 0, ',', '.')); ?></p>
                    </div>
                </div>
                ...
                <div class="bg-white/70 dark:bg-surface-700/50 border border-surface-200 dark:border-surface-600 rounded-xl px-4 py-3 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-extrabold text-surface-700 dark:text-surface-200 leading-tight"><?php echo e($totalAllCount); ?> <span class="text-sm font-normal text-surface-400">transaksi</span></p>
                        <p class="text-xs text-surface-400"><?php echo e($approvedCount); ?> disetujui</p>
                    </div>
                </div>
        ...
        <div class="bg-gradient-to-r from-brand-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 border border-brand-100 dark:border-brand-800 rounded-xl shadow-sm p-5">
            ...
            ?>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <?php $__currentLoopData = $grupItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $g): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="<?php echo e($g['boxBg']); ?> border <?php echo e($g['boxBorder']); ?> rounded-xl px-4 py-3 flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg <?php echo e($g['iconBg']); ?> flex items-center justify-center shrink-0">
                        <svg class="w-5 h-5 <?php echo e($g['iconColor']); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <?php echo $g['icon']; ?>

                        </svg>
                    </div>
                    <div class="min-w-0 flex-1">
                        <p class="text-xs <?php echo e($g['subColor']); ?> font-medium"><?php echo e($g['label']); ?></p>
                        <p class="text-xl font-extrabold <?php echo e($g['valColor']); ?> leading-tight truncate">
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            <div class="flex-1 h-2 bg-surface-200 dark:bg-surface-600 rounded-full overflow-hidden">
                                <div class="h-full rounded-full transition-all duration-500"
                                     style="width: <?php echo e($grandTotal > 0 ? number_format($g['total'] / $grandTotal * 100, 1, '.', '') : 0); ?>%; background-color: <?php echo e($g['barHex']); ?>;"></div>
                            </div>
                            <span class="text-xs font-bold <?php echo e($g['subColor']); ?> shrink-0">
                                <?php echo e($grandTotal > 0 ? number_format($g['total'] / $grandTotal * 100, 1) : 0); ?>%
                            </span>
                        </div>
                    </div>
                </div>
        ...
        <?php if($totalAllCount > 0 && !$filterDate): ?>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="lg:col-span-2 bg-white dark:bg-surface-800 rounded-2xl border border-surface-200 dark:border-surface-700 shadow-sm p-5">
                <h3 class="text-sm font-semibold text-surface-700 dark:text-surface-100 mb-4">
                    📊 Grafik Pengeluaran Harian — <?php echo e($months[$month]); ?> <?php echo e($year); ?>
            ...
            <div class="bg-white dark:bg-surface-800 rounded-2xl border border-surface-200 dark:border-surface-700 shadow-sm overflow-hidden"
                 x-data="{
                     rows: <?php echo e(Illuminate\Support\Js::from($perJenis)); ?>,
                     sortCol: 'total',
                     sortDir: 'desc',
                     sort(col) {
                         if (this.sortCol === col) {
                     ...
                     totalAll: <?php echo e($totalPengeluaran ?? 0); ?>,
                 }">
        ...
        <div class="bg-white dark:bg-surface-800 rounded-2xl border border-surface-200 dark:border-surface-700 shadow-sm overflow-hidden">
            <div class="px-5 py-3 border-b border-surface-200 dark:border-surface-700 flex items-center justify-between">
                ...
                <span class="text-xs text-surface-400 dark:text-surface-500"><?php echo e($totalAllCount); ?> transaksi</span>
            </div>

            
            <div class="divide-y divide-surface-100 dark:divide-gray-700 block sm:hidden">
                <?php $__empty_1 = true; $__currentLoopData = $records; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tx): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="px-4 py-3">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                            <p class="text-sm font-semibold text-surface-800 dark:text-surface-100 truncate"><?php echo e($tx->transactionType->name); ?></p>
                            <p class="text-xs text-surface-400 dark:text-surface-500 mt-0.5">
                                <?php echo e($tx->transaction_date->isoFormat('ddd, D MMM Y')); ?>

                            </p>
                            <?php if($tx->description): ?>
                            <p class="text-xs text-surface-500 dark:text-surface-400 mt-0.5 truncate"><?php echo e($tx->description); ?></p>
                            <?php endif; ?>
                        </div>
                        <div class="text-right flex-shrink-0">
                            <p class="text-sm font-bold text-red-600 dark:text-red-400">Rp <?php echo e(number_format($tx->amount, 0, ',', '.')); ?></p>
                            <span class="inline-block mt-1 px-2 py-0.5 text-xs rounded-full font-medium
                                <?php echo e($tx->status === 'approved' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400'
                ...
                <div class="px-5 py-10 text-center text-surface-400 dark:text-surface-500 text-sm">
                    Belum ada pengeluaran untuk <?php echo e($months[$month]); ?> <?php echo e($year); ?>.
                </div>
                <?php endif; ?>
            </div>
                    ...
                    <tbody class="divide-y divide-surface-100 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-surface-50 dark:hover:bg-surface-700/50 transition-colors">
                            <td class="px-5 py-3 text-surface-700 dark:text-surface-300 whitespace-nowrap">
                                <p class="font-medium"><?php echo e($tx->transaction_date->format('d M Y')); ?></p>
                                <p class="text-xs text-surface-400 dark:text-surface-500"><?php echo e($tx->transaction_date->isoFormat('dddd')); ?></p>
                            </td>
                            <td class="px-5 py-3 text-surface-800 dark:text-surface-100 font-medium">
                                <?php echo e($tx->transactionType->name); ?>
                            ...
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <?php if(in_array($tx->status, ['draft','pending','rejected'])): ?>
                        ...
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center text-surface-400 dark:text-surface-500">
                                Belum ada pengeluaran untuk <?php echo e($months[$month]); ?> <?php echo e($year); ?>.
                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('create transactions')): ?>
                                <a href="<?php echo e(route('transactions.create')); ?>" class="text-brand-600 dark:text-brand-400 hover:underline ml-1">+ Tambah sekarang</a>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endif; ?>
                    ...
                    <tfoot class="bg-surface-50 dark:bg-surface-700/50 border-t-2 border-surface-200 dark:border-surface-600">
                        <tr>
                            <td colspan="2" class="px-5 py-3 text-sm font-semibold text-surface-600 dark:text-surface-300">
                                Total Disetujui (<?php echo e($approvedCount); ?> transaksi)
                            </td>
                            <td class="px-5 py-3 text-right font-bold text-red-600 dark:text-red-400 whitespace-nowrap">
            ...
            <div class="px-5 py-4 border-t dark:border-surface-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <p class="text-xs text-surface-500 dark:text-surface-400">
                    Menampilkan <?php echo e($records->firstItem()); ?>–<?php echo e($records->lastItem()); ?> dari <?php echo e($records->total()); ?> transaksi
                </p>
                <div>
    ...
    (function () {
        ...
        const data   = <?php echo e(Illuminate\Support\Js::from($chartData)); ?>;
...
<?php endif; ?>
<?php /**PATH /Users/deniubaidillah/Documents/Project/Sistem_Keuangan_clean/resources/views/pengeluaran/index.blade.php ENDPATH**/ ?>